...
- **`>` is an explicit project / session delimiter.** Typing `pok > add` matches sessions whose project name fuzzy-matches `pok` AND whose session/worktree name fuzzy-matches `add`. Each half is scored independently with `defaultFilter`, against `keywords[0]` (project) and `keywords[1]` (session + worktree). The right half may be empty (`foo >` = match all sessions inside `foo*` projects).
- **Bare-left `> foo` scopes to the active project.** When the left half is empty, the palette gates results to the currently selected project (id from `useProjectsStore`, matched against `keywords[2]`). If no project is selected (e.g., palette opened from a global view), the project gate is dropped and behavior reduces to "search session/worktree across all projects" so the palette never silently goes empty.

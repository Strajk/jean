...
// reads `selectedProjectId` directly to scope results to the active project.
...
export function SessionPalette() {
  ...
  // scorer, against `keywords[0]` (project name) and `keywords[1]` (session +
  // worktree) respectively. Without `>`, fall back to cmdk's default behavior
  // so single-token searches keep working as before.
  //
  // Special case: a bare `>` (empty left half) scopes the search to the
  // currently selected project — `keywords[2]` carries the project id and we
  // gate on exact-id match. If nothing is selected, the project gate is
  // dropped and behavior reduces to "search session/worktree across all
  // projects" so the palette never silently goes empty.
  const filter = useCallback(
    (value: string, search: string, keywords?: string[]): number => {
      ...
      const itemProjectId = keywords?.[2] ?? ''

...
interface GitStatusBadgesProps {
  ...
  /** Incoming commits (origin/base ahead of HEAD) — shown in the pull tooltip. */
  incomingCommits?: { hash: string; message: string }[]
  diffAdded: number
  diffRemoved: number
      ...
      {behindCount > 0 && (
        <Tooltip>
          ...
          <TooltipContent className="max-w-80">
            <p>{`Merge ${behindCount} new commit${behindCount > 1 ? 's' : ''} from remote base branch`}</p>
              ...
              <ul className="mt-1 space-y-0.5 border-t border-border/50 pt-1">
                ...
                {behindCount > incomingCommits.length && (
                  <li className="text-[11px] text-muted-foreground">
                    ...and {behindCount - incomingCommits.length} more
                  </li>
                )}
              </ul>
            )}
